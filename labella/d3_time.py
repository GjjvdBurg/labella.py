...
daysThisMonth = lambda x: (
    x.replace(month=x.month % 12 + 1, day=1) - timedelta(days=1)
).day
...
d3_time["second"] = d3_time_interval(
    ...
    lambda date, offset: milli2dt((dt2milli(date) + math.floor(offset) * 1e3)),
...
d3_time["minute"] = d3_time_interval(
    ...
    lambda date, offset: milli2dt(dt2milli(date) + math.floor(offset) * 6e4),
...
def d3_time_hour_local(date):
    ...
    ndate = milli2dt(
        (math.floor(dt2milli(date) / 36e5 - timezone) + timezone) * 36e5
    )
    return ndate
...
d3_time["hour"] = d3_time_interval(
    ...
    lambda date, offset: milli2dt(dt2milli(date) + math.floor(offset) * 36e5),
...
d3_time["week"] = d3_time_interval(
    ...
    lambda date, offset: datetime.fromtimestamp(
        date.timestamp() + math.floor(offset) * 7 * 24 * 3600
    ),
